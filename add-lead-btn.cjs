...
if(!src.includes('AddLeadModal')) {
    src = src.replace('const LeadDetailModal = ({', addLeadModalStr + '\n\nconst LeadDetailModal = ({');
    ...
    // add button in UI
    const buttonHtml = `        <button onClick={() => setShowAddLead(true)} className="flex flex-1 sm:flex-none justify-center items-center gap-2 px-4 py-2 bg-[#ff0000] hover:bg-[#ff3333] text-black rounded-lg transition-colors text-sm font-bold uppercase tracking-widest">
          <Plus size={16} /> Add Lead
        </button>`;
        
    src = src.replace('<button onClick={() => setViewMode(viewMode === \'pipeline\' ? \'analytics\' : \'pipeline\')}', buttonHtml + '\n        <button onClick={() => setViewMode(viewMode === \'pipeline\' ? \'analytics\' : \'pipeline\')}');
    
    // add modal to render
    src = src.replace('{selectedLead && (', '{showAddLead && <AddLeadModal onClose={() => setShowAddLead(false)} onSuccess={() => setShowAddLead(false)} />}\n      {selectedLead && (');
