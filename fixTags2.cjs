...
// Remove anything below `</AnimatePresence>` from original and replace
let idx = code.indexOf('</AnimatePresence>\n\n                          </div>');
if (idx === -1) {
    idx = code.indexOf('</AnimatePresence>');
}
if (idx !== -1) {
   code = code.substring(0, idx) + fixedPart;
